...
export function runFixedSpreadBacktest(seed:number,spread:number,steps=120,hedged=false,adverseAware=false,overrides:Partial<BacktestConfig>={}):BacktestResult{
  ...
  for(let t=0;t<steps;t++){
    ...
    const bidOrder=exchange.submit("MM","BUY","LIMIT",1,Math.floor((mid-quoteSpread)/.01)*.01);const askOrder=exchange.submit("MM","SELL","LIMIT",1,Math.ceil((mid+quoteSpread)/.01)*.01);bid=bidOrder.status==="REJECTED"?undefined:bidOrder.id;ask=askOrder.status==="REJECTED"?undefined:askOrder.id;submitted+=2;
    ...
    if(flow()<hitProbability){
      ...
      if(adverseAware){const imbalance=clamp((flow()-.5)*2+.35*regime,-1,1);const volatilityChange=Math.min(1,Math.abs(mid-(path[Math.max(0,t-1)]??mid))/mid*10);lastEvidence={side,imbalance,volatilityChange};}
      exchange.submit("FLOW",side,"MARKET",size);
    }
    const fillsBefore=exchange.fills.length;exchange.advanceTo(t+1);const newFills=exchange.fills.slice(fillsBefore).filter(f=>f.maker==="MM");const nextMark=path[Math.min(t+2,path.length-1)];newFills.forEach(f=>markouts.push(exchange.orders.get(f.makerOrderId)?.side==="BUY"?nextMark-f.price:f.price-nextMark));maxAbs=Math.max(maxAbs,Math.abs(exchange.position("MM")));
    const optionDelta=exchange.position("MM")*bs({spot:mid,strike:100,maturity:Math.max((steps-t)/steps,1/365),vol:config.vol,rate:.05,dividend:0,paths:1,seed}).delta*exchange.config.multiplier;
    if(hedged){const target=-Math.round(optionDelta);const trade=target-hedgeQty;if(trade!==0){hedgeCash-=trade*mid;hedgeQty+=trade;const fee=Math.abs(trade)*.01;hedgeCash-=fee;hedgeFees+=fee;}maxAbsDelta=Math.max(maxAbsDelta,Math.abs(optionDelta+hedgeQty));}else maxAbsDelta=Math.max(maxAbsDelta,Math.abs(optionDelta));
  }
  exchange.advanceTo(steps+2);const finalMid=path.at(-1)??100;const optionPnl=exchange.equity("MM",finalMid);const hedgePnl=hedgeCash+hedgeQty*finalMid;const netPnl=optionPnl+hedgePnl;const mmFills=exchange.fills.filter(f=>f.maker==="MM");const reconciles=Math.abs(optionPnl-(exchange.cash("MM")+exchange.position("MM")*finalMid*exchange.config.multiplier))<1e-8;
  return {seed,spread,hedged,adverseAware,netPnl,optionPnl,hedgePnl,fees:exchange.fees("MM"),hedgeFees,fills:mmFills.reduce((sum,f)=>sum+f.quantity,0),fillRate:submitted?mmFills.reduce((sum,f)=>sum+f.quantity,0)/submitted:0,maxAbsInventory:maxAbs,maxAbsDelta,meanQuotedSpread:spreadSum/steps,meanAdverseProbability:adverseSum/steps,finalInventory:exchange.position("MM"),finalHedge:hedgeQty,finalMid,meanMarkout:markouts.length?markouts.reduce((a,b)=>a+b,0)/markouts.length:0,reconciles,flowBuyRate:arrivals?buyFills/arrivals:0,flowArrivalRate:arrivals/steps,flowRegimePersistence:config.flowPersistence};
}
